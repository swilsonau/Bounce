      ...
      while($prog = mysqli_fetch_array($checkpermssql)) {
        ...
        if($prog['type'] == '1') {
          ...
          for($i = 0; $i < $numofdays; $i++) {
              ...
              $array[] = array("id" => $prog['pid'], "title" => $prog['programname'], "start" => date('Y-m-d\TG:i', $timestart), "end" => date('Y-m-d\TG:i', $timeend), "color" => $color);
          }
        } else {
          // One off
          ...
          $array[] = array("id" => $prog['pid'], "title" => "One off: ".$prog['programname'], "start" => date('Y-m-d\TG:i', $timestart), "end" => date('Y-m-d\TG:i', $timeend), "color" => $color);
        }
      }
  }
