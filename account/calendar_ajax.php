...
$checkpermssql = mysqli_query($sql, "SELECT *, `programs`.`id` AS `pid` FROM `programs` LEFT JOIN `program_assign` ON `program_assign`.`progid` = `programs`.`id` WHERE `program_assign`.`userid` = '$userid'");
      ...
      while($prog = mysqli_fetch_array($checkpermssql)) {
        if($prog['type'] == '1') {
          ...
          for($i = 0; $i < $numofdays; $i++) {
              ...
              $array[] = array("id" => $prog['pid'], "title" => $prog['programname']. " $i", "start" => date('Y-m-d\TG:i', $timestart), "end" => date('Y-m-d\TG:i', $timeend));
          }
        } else {
          // One off
          ...
          $array[] = array("id" => $prog['pid'], "title" => "One off: ".$prog['programname'], "start" => date('Y-m-d\TG:i', $timestart), "end" => date('Y-m-d\TG:i', $timeend));
        }
      }
  }
